...
sample_length <- 500
idx_dates <- which(returns_ds$bric_date %in% returns_cripto$btc_date)
all_common_dates <- returns_ds$bric_date[idx_dates]

w_opt_list <- list()
all_names <- instr#append(instr,c("btc"))

allocations <- NULL

for(i in 1:(length(all_common_dates)-sample_length)){
  common_dates <- all_common_dates[i:(i+sample_length)]
  ...
  allocations <- rbind(allocations,opt_w_no_crypto)
}

rownames(allocations) <- all_common_dates[(sample_length+1):length(all_common_dates)]
colnames(allocations) <- all_names

for(nome in colnames(allocations)){
  for(date in rownames(allocations)){
    if(allocations[date,nome]<1e-10){
      allocations[date,nome] <- 0
    }
  }
}


allocations_d <- as.data.frame(allocations)
allocations <- as.data.frame(allocations)
allocations_d$dates <- rownames(allocations_d)

library(reshape2)
final_df <- melt(allocations_d, id="dates")
# 
# 
# 
# library(ggplot2)
# library(RColorBrewer)
# ggplot(final_df, aes(x=dates,y=value, fill=variable)) +
#   geom_area(alpha=1 , size=1, colour="black") +
#   ggtitle("Markowitz Optimal Allocation Without Shortsellling")+
#   scale_fill_manual(values = colorRampPalette(brewer.pal(9, "Paired"))(length(colnames(allocations))) )
# 
rownames(allocations) <- all_common_dates[(sample_length+1):length(all_common_dates)]
colnames(allocations) <- all_names


# write.csv(file = "C:/Users/matte/Desktop/papers/tesi/cap 5/allocations.csv", x = allocations)
